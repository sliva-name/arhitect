...
В Nuxt 4 весь код приложения находится в директории `app/`:
...
│   └── tasks.vue    # Страница задач (/tasks)
└── composables/     # Композаблы (переиспользуемая логика)
    └── useTasks.ts  # Пример композабла
```
...
- ✅ Автоматическая инициализация Laravel и Nuxt проектов
- ✅ Правильные права доступа к файлам (не root)
- ✅ Готовый CRUD пример (Tasks)
- ✅ TypeScript поддержка в Nuxt
- ✅ PHPStan и Pint для качества кода
